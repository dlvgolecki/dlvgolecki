 ...
 */
require ELSTON_ADMIN_PATH .'/cs-framework.php';
require ELSTON_FRAMEWORK .'/theme-options/theme-options.php';
require ELSTON_FRAMEWORK .'/theme-options/theme-customizer.php';
require ELSTON_FRAMEWORK .'/theme-options/theme-metabox.php';
 ...
 */
function elston_cs_framework_override() {
  return 'inc/theme-options/theme-extend';
}
add_filter( 'cs_framework_override', 'elston_cs_framework_override' );

/**
 * Add Extra Icons Font in Icon Picker
 */
function elston_add_icons_json( $json = array() ) {
  $json[] = ELSTON_CS_FRAMEWORK .'/fields/icon/simple-line-icons.json';
  return $json;
}
add_filter( 'cs_add_icons_json', 'elston_add_icons_json' );

/**
 * Custom New Font Family
 */
if( ! function_exists( 'elston_custom_font_upload' ) ) {
  function elston_custom_font_upload( $db_value ) {
  ...
  add_action( 'cs_typography_family', 'elston_custom_font_upload' );
}

/**
 * Check Custom Font
 */
if ( ! function_exists( 'elston_custom_upload_font' ) ) {
  function elston_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'elston_registered_sidebars' ) ) {
  function elston_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'elston_google_fonts' ) ) {
  function elston_google_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'elston-google-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'elston_get_typography' ) ) {
  function elston_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? elston_esc_string( $font['font']['variant'] ) : 400;
          $style   = elston_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. elston_check_px( $font['size'] ) .';' : '';
