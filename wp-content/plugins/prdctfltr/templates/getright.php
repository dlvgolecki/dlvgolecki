<?php
	...
	switch ( XforWC_Product_Filters_Frontend::$settings['template'] ) {
		...
		case 'loop/pagination.php' :
			...
			if ( $pf_pag_type == 'prdctfltr-pagination-default' ) {
				...
				if ( Prdctfltr()->version_compare( '3.6.0' ) ) {
					...
					<nav class="woocommerce-pagination prdctfltr-pagination prdctfltr-pagination-default">
						<?php
							echo paginate_links( apply_filters( 'prdctfltr_pagination_args', array(
								...
								'format'       => '',
								'add_args'     => false,
								'current'      => max( 1, !isset( $wp_query->query_vars['paged'] ) ? XforWC_Product_Filters_Shortcodes::$settings['instance']->query_vars['paged'] : $wp_query->query_vars['paged'] ),
								'total'        => !isset( $wp_query->max_num_pages ) ? XforWC_Product_Filters_Shortcodes::$settings['instance']->max_num_pages : $wp_query->max_num_pages,
								'prev_text'    => '&larr;',
								'next_text'    => '&rarr;',
								'type'         => 'list',
								'end_size'     => 3,
			...
			?>
				...
				<?php
					...
					?>
						<span class="button disabled"><?php esc_html_e( 'No products found', 'prdctfltr' ); ?></span>
					<?php
					}
				?>
				</nav>
	...
	/*	case 'loop/no-products-found.php' :
			...
			XforWC_Product_Filters_Frontend::$settings['did_noproducts'] = true;

			if ( isset( XforWC_Product_Filters_Frontend::$settings['instance'] ) ) {

				if ( !empty( XforWC_Product_Filters_Frontend::$settings['instance']['general']['no_products'] ) ) {

					echo '<div class="' . esc_attr( empty( XforWC_Product_Filters_Frontend::$options['install']['ajax']['wrapper'] ) ? 'products' : str_replace( '.', '', XforWC_Product_Filters_Frontend::$options['install']['ajax']['wrapper'] ) ) . ' woocommerce-page prdctfltr-added-wrap">';

						echo do_shortcode( wp_kses_post( XforWC_Product_Filters_Frontend::$settings['instance']['general']['no_products'] ) );
