			...
			<div>
				...
				<p class="prdctfltr-box">
					...
					<select name="<?php echo esc_attr( $this->get_field_name('preset') ); ?>" id="<?php echo esc_attr( $this->get_field_id('preset') ); ?>" class="widefat">
						<option value="pf_default_inline"<?php echo ( $preset == 'pf_default_inline' ? ' selected="selected"' : '' ); ?>><?php esc_html_e('Flat inline', 'prdctfltr' ); ?></option>
						<option value="pf_default"<?php echo ( $preset == 'pf_default' ? ' selected="selected"' : '' ); ?>><?php esc_html_e('Flat block', 'prdctfltr' ); ?></option>
						<option value="pf_default_select"<?php echo ( $preset == 'pf_default_select' ? ' selected="selected"' : '' ); ?>><?php esc_html_e('Flat select', 'prdctfltr' ); ?></option>
					</select>
				</p>
				...
				<p class="prdctfltr-box">
					...
					<?php
						$options = get_option( '_prdctfltr_autoload', false );
						if ( $options === false && get_option( 'wc_settings_prdctfltr_version', false ) !== false ) {
							$curr_templates = get_option( 'prdctfltr_templates', false );
							if ( $curr_templates === false ) {
								$array = array();
							}
							else {
								foreach ( $curr_templates as $k => $v ) {
								?>
									<option value="<?php echo esc_attr( $k ); ?>"<?php echo ( $template == $k ? ' selected="selected"' : '' ); ?>><?php echo esc_html( $k ); ?></option>
								<?php
								}
							}
						}
							...
							if ( isset( $options['presets'] ) && is_array( $options['presets'] ) ) {
								foreach ( $options['presets'] as $k => $v ) {
								?>
