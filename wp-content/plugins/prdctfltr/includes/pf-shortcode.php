<?php
	...
	class XforWC_Product_Filters_Shortcodes {
		...
		public static function get_wp_query( $query ) {
			...
			if ( !XforWC_Product_Filters::version_check( '3.1.9' ) ) {
	...
	$('.prdctfltr-fix-wcsc:first').remove();
		...
		public static function add_wcsc_filter( $atts ) {
			if ( !empty( $atts['prdctfltr'] ) ) {
				...
				if ( !XforWC_Product_Filters::version_check( '3.1.9' ) ) {
					add_action( 'loop_start', 'XforWC_Product_Filters_Shortcodes::get_wp_query' );
				}
				...
				if ( XforWC_Product_Filters::version_check( '3.2.0' ) ) {
				...
				if ( !empty( $wp_query ) ) {
					...
					if ( !in_array( 'result', $ajaxTemplates ) && isset( $opt['pf_count_template'] ) ) {
						...
						if ( $wp_query->found_posts > 0 ) {
							...
							wc_set_loop_prop( 'current', max( 1, intval( XforWC_Product_Filters_Frontend::$options['sc_instance']['paged'] ) ) );
