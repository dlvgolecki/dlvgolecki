	...
	$(document).on( 'svx-filters-save', function(e,f) {
		...
		$.each( r, function(i,b) {
			...
			$.each( svx.settings.filters.settings[b.type], function(n,p) {
				s[i][n] = u(svx.settings.filters.val[i][n])?svx.settings.filters.val[i][n]:(u(svx.settings.filters.settings[b.type].default)?svx.settings.filters.settings[b.type].default:'');
			} );
	...
	function __check_style(b,t) {
		if ( ___check_load(t) ) {
			...
			if ( u(b.style) || u(b.options) ) {
				...
				if ( $.inArray( u(o.style.type), [ '', 'text', 'color', 'image', 'select', 'html' ] ) !== -1 ) {
					b.style = o;
				}
