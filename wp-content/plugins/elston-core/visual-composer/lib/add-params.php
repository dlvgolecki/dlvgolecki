...
if(!class_exists('VT_Notice_Field'))
{
	class VT_Notice_Field
	{
		...
		function notice_settings_field( $settings, $value )
		{
			...
			$output = '<div class="cs-field-'. $type .' '. $param_name .'"><div class="cs-'. $type .' '. $class .'">'. $param_heading .'</div><div class="clear"></div></div>';
			return $output;
		}
...
if( class_exists( 'VT_Notice_Field' ) )
{
	$evltNoticeParam = new VT_Notice_Field();
}

/* 1. Custom Taxonomy */
if(!class_exists('VT_Wlsn_Taxonomy_Field'))
{
	class VT_Wlsn_Taxonomy_Field
	{
		function __construct()
		{
			if( function_exists( 'vc_add_shortcode_param' ) )
			{
				vc_add_shortcode_param( 'elstn_taxonomy' , array( &$this, 'elstn_taxonomy_settings_field' ) );
			}
		}

		function elstn_taxonomy_settings_field( $settings, $value )
		{
			$param_name = isset($settings['param_name']) ? $settings['param_name'] : '';
			...
			$categories = '<select name="'.$param_name.'" class="wpb_vc_param_value wpb-input wpb-select '.$param_name.' dropdown '.$value.'" data-option="'.$value.'">';
			$categories .= '<option class="' . $category->slug . '" value="">Select Category</option>';

			foreach( $terms as $category ){
			  $checked = ($value == $category->term_id) ? 'selected' : '';
			  $categories .= '<option class="' . $category->slug . '" value="' . $category->term_id . '" '.$checked.'>' . $category->name . '</option>';
			}
			$categories .= '</select>';
...
if( class_exists( 'VT_Wlsn_Taxonomy_Field' ) )
{
	$evltNoticeParam = new VT_Wlsn_Taxonomy_Field();
}
...
if(!class_exists('VcSwitcherField'))
{
	class VcSwitcherField {
		...
		public function vc_enqueue_editor_scripts_befe() {
			wp_enqueue_script( 'core-scripts', ELSTN_PLUGIN_ASTS . '/core-scripts.js' );
		}
...
if( class_exists( 'VT_Icon_Picker_Field' ) )
{
	$evltVtIconParam = new VT_Icon_Picker_Field();
}
