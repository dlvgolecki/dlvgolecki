...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('elstn_gmap_function')) {
  function elstn_gmap_function( $atts, $content = NULL ) {
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = ELSTN_PLUGIN_ASTS . '/images/map-marker.png';
    }
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.elstn-gmap-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. elstn_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }

    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' elstn-gmap-'. $e_uniqid;

    wp_enqueue_script( 'elston-googlemap', ELSTN_PLUGIN_ASTS . '/jquery.googlemap.js', array( 'jquery' ), '1.5.0', true );
    wp_enqueue_script( 'elston-map-api', '//maps.googleapis.com/maps/api/js'. $gmap_api .'', array( 'jquery' ), '', true );
    ...
    // Map Tab ID
    if ($gmap_id) {
      $gmap_open = '<div '. $gmap_id .' class="elstn-gmap-tab tab-pane fade">';
      $gmap_close = '</div>';
    } else {
      $gmap_open = '';
      $gmap_close = '';
    ...
    echo $gmap_open;
    echo '<div id="map-'. $e_uniqid .'" class="elstn-custom-map '. $custom_css . $class . $styled_class .'"></div>';
    echo $gmap_close;
...
add_shortcode( 'elstn_gmap', 'elstn_gmap_function' );
