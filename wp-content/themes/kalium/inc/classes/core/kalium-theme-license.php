...
class Kalium_Theme_License {
	...
	public function admin_init() {
		...
		if ( 'kalium-product-registration' == $this->admin_page && isset( $_GET['license_key'] ) ) {
			...
			if ( kalium()->url->get( 'perform_verification' ) ) {
				$verify_license_response = wp_remote_get( self::getAPIServerURL() . "/verify-license/{$license_key}/" );
	...
	public function productRegistrationMenuItem() {
		...
		add_submenu_page( 'laborator_options', 'Product Registration', 'Product Registration', 'edit_theme_options', 'kalium-product-registration', array(
			$this,
			'adminPageProductRegistration',
		) );
	}
	...
	public static function getLicenseKey() {
		return self::$license ? self::$license->license_key : '';
	}
