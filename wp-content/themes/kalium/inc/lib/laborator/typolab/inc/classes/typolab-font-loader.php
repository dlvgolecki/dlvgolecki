...
class TypoLab_Font_Loader extends TypoLab {
	...
	public function enqueueFonts( $font_source, $font_placement, $fonts ) {
		switch ( $font_source ) {
			...
			// TypeKit Enqueue
			case 'typekit':
				$typekit_legacy_fonts = $adobe_fonts = array();

				foreach ( $fonts as $font ) {
					if ( false === TypoLab::isAdobeFont( $font ) ) {
				...
				$this->enqueueFontsFromTypeKit( $typekit_legacy_fonts, $font_placement );
				$this->enqueueFontsFromCustomFonts( $adobe_fonts, $font_placement );
				break;

			// Custom Font
			case 'uploaded-font':
				$this->enqueueFontsFromUploadFonts( $fonts, $font_placement );
				break;
	...
	public function enqueueFontsFromGoogle( $fonts, $font_placement ) {
		...
		if ( $font_families ) {
			...
			$font_import_url = "{$google_fonts_url}?family={$font_families}&amp;subset={$font_subsets}";
	 ...
	 */
	public function enqueueFontsFromUploadFonts( $fonts, $font_placement ) {
		foreach ( $fonts as $font ) {
			...
			foreach ( $font_variants as $font_variant ) {
				echo TypoLab_Uploaded_Font::getFontFace( $font_family, $font_variant );
				echo PHP_EOL;
			}
			echo '</style>';
		}
