...
function share_story_network_link( $network, $post_id = null, $class = '', $icon = false ) {
	...
	$title     = esc_attr( urlencode( get_the_title( $post_id ) ) );
	$excerpt   = esc_attr( urlencode( $excerpt ) );
	$permalink = esc_attr( urlencode( get_permalink( $post_id ) ) );
	...
	?>
    <a class="<?php echo esc_attr( trim( "{$network_entry['icon']} {$class}" ) ); ?>"
       href="<?php echo $network_entry['url']; ?>"<?php when_match( $new_window, 'target="_blank"' ); ?>>
		<?php if ( $icon ) : ?>
...
function is_shop_supported() {
	return kalium()->helpers->isPluginActive( 'woocommerce/woocommerce.php' ) || class_exists( 'WooCommerce' );
}
...
function is_acf_pro_activated() {
	return kalium()->helpers->isPluginActive( 'advanced-custom-fields-pro/acf.php' );
}
...
function kalium_get_custom_skin_file_path( $absolute = false ) {
	...
	$relative_theme_path  = substr( get_stylesheet_directory(), strlen( ABSPATH ) );
	...
	// Absolute path
	if ( $absolute ) {
		$custom_skin_filepath = ABSPATH . $custom_skin_filepath;
	}
