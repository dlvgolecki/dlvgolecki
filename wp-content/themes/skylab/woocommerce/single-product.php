...
get_header('shop'); ?>
	...
	<?php if ( apply_filters( 'woocommerce_show_page_title', true ) ) : ?>
		...
		<?php if ( empty( $color_for_heading ) ) { ?>
			<?php $color_for_heading = '#444444'; ?>
		<?php } ?>
	
		<div class="entry-header-wrapper">
			<header class="entry-header clearfix" <?php if ( $background_overlay_color_for_heading != '#ffffff' || $background_overlay_opacity_for_heading != '.1' ) { ?>style="<?php if ( $background_overlay_color_for_heading != '#ffffff' ) { ?>background: <?php echo esc_attr( $background_overlay_color_for_heading ); ?>;<?php } ?> <?php if ( $background_overlay_opacity_for_heading != '.1' ) { ?>background: <?php echo esc_attr( $rgba ); ?>;<?php } ?>"<?php } ?>></header><!-- .entry-header -->
		</div>
	<?php endif; ?>
